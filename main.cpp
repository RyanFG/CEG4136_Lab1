...
//__global__ void forestColour(int* forest_GPU, int* burn_GPU) {
//    float cellSize = 2.0f / N;
//    int gid = threadIdx.x + blockIdx.x * blockDim.x;
//
//    if (forest_GPU[gid] == 0 && burn_GPU[gid] == 0) {
//        glColor3f(0.8f, 0.8f, 0.8f);  // Empty space (gray)
//    }
//    else if (forest_GPU[gid] == 1) {
//        glColor3f(0.0f, 1.0f, 0.0f); // Tree (green)
//    }
//    else if (forest_GPU[gid] == 2) {
//        glColor3f(1.0f, 0.0f, 0.0f);  // Tree on fire (red)
//    }
//    else if (forest_GPU[gid] == 3) {
//        glColor3f(0.0f, 0.0f, 0.0f);  // Burned tree (black)
//    }
//
//    // Draw the cell
//    float x = -1.0f + (blockIdx.x * blockDim.x) * cellSize;
//    float y = -1.0f + threadIdx.x * cellSize;
//    glBegin(GL_QUADS);
//    glVertex2f(x, y);
...
void drawForest() {
    ...
    //forestColour << <N, N >> > (forest_GPU, burnTime_GPU);
